...
function initializeVis_2(vis_container_id, scale=1000) {
  ...
  function clicked(d) {
    ...
    if (active.node() === this) {
      ...
      var state = x[x.length-1];
      test(vis_container_id, state);
...
function test(i, state) {
  var state_lat = cities_lat_lon[state].lat;
  var state_lon = cities_lat_lon[state].lon;
  
  d3.select("#vis_"+i+"_svg_div").append("div").attr("id", "map");
  
  // Create the Google Map…
  var map = new google.maps.Map(d3.select("#map").node(), {
    zoom: 7,
    center: new google.maps.LatLng(state_lat, state_lon),
